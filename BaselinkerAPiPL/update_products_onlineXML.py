...
def update_product_quantity_in_baselinker(products: List[Dict], storage_id: str, sku_to_id: Dict[str, str]) -> bool:
...
def update_product_prices_in_baselinker(products: List[Dict], storage_id: str, sku_to_id: Dict[str, str]) -> bool:
    """Aktualizuje ceny produktów w BaseLinker przez API."""
    headers = {"X-BLToken": API_TOKEN}
    formatted_products = []
    ...
    for product in products:
        ...
        if product_id != "0":  # Aktualizuj tylko jeśli produkt istnieje
            ...
            print(f"Aktualizacja ceny produktu: SKU={product['sku']}, Product ID={product_id}, Cena={product['price_brutto']}")
...
def process_batch(batch: List[Dict], queue: Queue, storage_id: str, sku_to_id: Dict[str, str]):
    ...
    if existing_products:
        ...
        success_quantity = update_product_quantity_in_baselinker(existing_products, storage_id, sku_to_id)
        # Następnie aktualizacja cen
        success_prices = update_product_prices_in_baselinker(existing_products, storage_id, sku_to_id)
...
def update_products_from_xml():
    """Główna funkcja aktualizacji produktów z pliku XML online."""
    ...
    # Pobieranie kategorii (niepotrzebne dla aktualizacji, ale zachowujemy dla spójności)
    get_category_id()
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        for batch in batches:
            executor.submit(process_batch, batch, result_queue, storage_id, sku_to_id_cache)
