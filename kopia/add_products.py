...
def fetch_and_parse_xml() -> List[Dict]:
    ...
    try:
        ...
        for item in root.findall(".//item"):
            ...
            ean = (item.find("g:gtin", namespace).text if item.find("g:gtin", namespace) is not None else "").strip()
            ...
            logging.info(f"Parsowanie produktu: ERP_ID={erp_id}, SKU={mpn}, Brand={brand}, Title={title}, Sformatowana nazwa={formatted_name}, Cena={price_czk}, Stan={quantity}, EAN={ean}, Kategoria={category}, Image={image_link}")
...
def add_product_to_baselinker(product: Dict, storage_id: str, category_id: str, inventory_id: str) -> bool:
    ...
    logging.info(f"Pełny payload wysyłany do BaseLinker (addProduct): {json.dumps(formatted_product, ensure_ascii=False)}")
    print(f"Pełny payload wysyłany do BaseLinker (addProduct): {json.dumps(formatted_product, ensure_ascii=False)}")
    print(f"Wysyłanie produktu: SKU={product['sku']}, Nazwa={product['name']}, Cena brutto (CZK)={price_brutto_czk}, ERP_ID={erp_id}, Storage ID={storage_id}")
    logging.info(f"Wysyłanie produktu: SKU={product['sku']}, Nazwa={product['name']}, Cena brutto (CZK)={price_brutto_czk}, ERP_ID={erp_id}, Storage ID={storage_id}")
    params = {
        "method": "addProduct",
    ...
    try:
        ...
        if product_id and str(product_id) != "0" and str(product_id).lower() != "none":
            ...
            save_sku_to_id()
            logging.info(f"Pomyślnie dodano produkt: SKU={product['sku']}, Product ID={product_id_str}, ERP_ID={erp_id}, Storage ID={storage_id}")
            print(f"Pomyślnie dodano produkt: SKU={product['sku']}, Product ID={product_id_str}, ERP_ID={erp_id}, Storage ID={storage_id}")
            return True
        else:
