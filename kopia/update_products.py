...
def fetch_and_parse_xml() -> List[Dict]:
    ...
    try:
        ...
        for item in root.findall(".//item"):
            ...
            mpn = (item.find("g:mpn", namespace).text if item.find("g:mpn", namespace) is not None else "").strip()
            ...
            # Parsowanie marki
            brand = (item.find("g:brand", namespace).text if item.find("g:brand", namespace) is not None else "").strip()
...
def process_batch(batch: List[Dict], queue: Queue, storage_id: str, sku_to_id: Dict[str, str], inventory_id: str):
    ...
    if existing_products:
        ...
        success = success_quantity and success_prices and success_text_fields
